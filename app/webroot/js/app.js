...
jQuery(document).ready(function ($) {
    $('.slider-comment').slick({
        ...
        pauseOnHover: true,
        initialSlide: 0,
        prevArrow: '<span class="slick-prev slider-comment-nav--prev" aria-label="previous"></span>',
    ...
    $('.slider-news').slick({
        ...
        pauseOnHover: true,
        initialSlide: 0,
        prevArrow: '<span class="slick-prev slider-news slick-nav--prev" aria-label="previous"></span>',
...
jQuery(document).ready(function ($){
    ...
    if ($message.css('display') != 'block') {
        $message.animate({width: "show"}, 1000); 
 
        var firstClick = true;
        $(document).bind('click.myEvent', function(e) {
            if (!firstClick && $(e.target).closest('.s_input').length == 0) {
                $message.animate({width: "hide"}, 1000);
    ...
    if ($mob_part.css('display') != 'block') {
        $mob_part.animate({width: "show"}, 1000);
    };
    $('.mob_close').click(function(e){
    		$mob_part.animate({width: "hide"}, 1000);
    	});
    e.preventDefault();
    });
